...
def agalma_info_table_parse(agalmaDir, fileName, threads, mem):
        ...
        with open(fileName, 'r') as agalmaFile:
                for line in agalmaFile:
                        ...
                        transcriptomeFile = os.path.join(sl[0], sl[1])
                        # Format catalog command
                        cmd = os.path.join(agalmaDir, 'agalma') + ' -t ' + str(threads) + ' -m ' + str(mem) + 'G' + ' catalog insert --id ' + catID + ' --paths ' + transcriptomeFile + ' --species "' + species + '"'
                        if sl[4] != '.':
                                sl[4] = sl[4].strip(' ')
                                cmd += ' --ncbi_id ' + sl[4]
                        if sl[5] != '.':
                        ...
                        cmd = os.path.join(agalmaDir, 'agalma') + ' -t ' + str(threads) + ' -m ' + str(mem) + 'G' + ' import --id ' + catID
                        if sl[6] == 'aa':
                                cmd += ' --seq_type aa'
                        outCmds.append(cmd)
                        # Format translate command (if relevant)
                        if sl[6] == 'nucl':
                                cmd = os.path.join(agalmaDir, 'agalma') + ' -t ' + str(threads) + ' -m ' + str(mem) + 'G' + ' translate --id ' + catID
                                outCmds.append(cmd)
                        # Format annotate command
                        cmd = os.path.join(agalmaDir, 'agalma') + ' -t ' + str(threads) + ' -m ' + str(mem) + 'G' + ' annotate --id ' + catID
                        outCmds.append(cmd)
        return outCmds

#### USER INPUT SECTION
usage = """%(prog)s will help with running AGALMA transcriptome import and
annotate commands.
"""
