...
# MANUAL SETUP BELOW
## SETUP: Specify program location
HAPCUTDIR=/home/n8942188/various_programs/HapCUT2-1.3.3/build
...
for gene in "${GENES[@]}"; do
    ...
    gatk --java-options "-Xmx4g" HaplotypeCaller -R ${GENOMEDIR}/${GENOMEFILE} -I ${gene}.sorted.fix.bam -O ${gene}.vcf;
    # Run HAPCUT2 prepper "extractHAIRS"
    ${HAPCUTDIR}/extractHAIRS --bam ${gene}.sorted.fix.bam --VCF ${gene}.vcf --out ${gene}_fragment_file;
    # Run HAPCUT2 to assemble
    ${HAPCUTDIR}/HAPCUT2 --fragments ${gene}_fragment_file --VCF ${gene}.vcf --output ${gene}_haplotype_output_file;
    cd ..;
done
