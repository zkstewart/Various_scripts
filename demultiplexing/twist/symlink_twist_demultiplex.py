...
DIR_REGEX = re.compile(r"Plate([A-Z1-9]{1})[-_]{1}Pool[-_]{1}(\d{1,2})")
...
def parse_tsv(fileName):
    EXPECTED_FIRST = ["plate", "pool", "well", "sample"]
    ...
    with open(fileName, "r") as fileIn:
        ...
        for line in fileIn:
            ...
            if firstLine:
                ...
                plate, pool, well, sample = sl
                if " " in sample:
                    oldSample = sample
                    sample = sample.replace(" ", "_")
                    print(f"# Sample with whitespace '{oldSample}' changed to be '{sample}'")
                
                layoutDict.setdefault(plate, {})
                layoutDict[plate].setdefault(pool, {})
                layoutDict[plate][pool][well] = sample
    return layoutDict

def main():
    ##### USER INPUT SECTION
    usage = """%(prog)s parses the sequencing plate layout as derived from
    demultiplex_twist_pools.sh and creates symlinks
