        ...
        for seqid in idDict:
            ...
            if foundseqid == None and effort == True:
                idMatches = [
                    ...
                    if record.long_name.startswith(seqid)
                ]
                if len(idMatches) == 1:
            ...
            # Otherwise write sequence to file
                ...
                fileOut.write('>' + record.long_name + '\n' + str(record) + '\n')
...
def remove(fastaRecords, longIndex, outputFileName, idDict, idShortDict, effort, effortPairs, foundList):
    with open(outputFileName, 'w') as fileOut:
        for record in fastaRecords:
            ...
            foundseqid = None
            if record.name in idDict:
                foundseqid = record.name
            elif record.long_name in idDict:
                foundseqid = record.long_name
            elif idShortDict != None:
                if record.name in idShortDict:
                    foundseqid = idShortDict[record.name]
            
            # If relevant, put more effort into finding ID in our idDict
            if foundseqid == None and effort == True:
                idMatches = [seqid for seqid in idDict if record.long_name.startswith(seqid)]           # This will return all entries in the idDict that partially match the current record's long name
                cleanMatches = []
                ...
                if cleanMatches != []:
                    if len(cleanMatches) == 1:
                        foundseqid = record.name             # This is probably the best way to handle partial matches through effort; it's likely to be a shortened version of the seqid if truncated, so we won't use the long_name
...
def pyfaidx_long_indexing(pyfaidxRecords):
    ...
    for record in pyfaidxRecords:
        longName = record.long_name.rstrip('\r') # pyfaidx doesn't seem to handle Windows-style newlines properly
        if longName in longIndex:
            return {} # can't meaningfully index if we have duplicate long names
        longIndex[longName] = record.name
    return longIndex
...
def main():
    ...
    records = Fasta(args.fastaFileName)
    longIndex = pyfaidx_long_indexing(records)
            ...
            for entry in idList:
                ...
                # If relevant, put more effort into finding ID in our idList
                    ...
                    if len(partialMatches) == 0:
                        ...
                        clipboardList.append(str(records[partialMatches[0]]))
                    else:
