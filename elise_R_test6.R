...
# Perform statistics
## Test 6: growth before VERSUS growth after for quiz responses
### Find files
currdir = list.files()
i = 1
quizcat_files = list()
for (val in currdir){
  ...
  else if (grepl('quizcat_div_compare', val)){
    quizcat_files[[i]] = val
    i = i + 1
  }
}
...
quizcat_wcx_test6 = list()
quizcat_lm_test6 = list()
...
for (file in quizcat_files){
  ...
  mdl = lm(species_num ~ treatment*quiz_response, data = quizData)
  #quizcat_wcx_test6[[i]] = fit
  quizcat_lm_test6[[i]] = mdl
  i = i + 1
}

## Linear model output - should be done with combined responses!
i = 1
for (stat in quizcat_lm_test6) {
...
write.csv(mainDF, file = paste(outdir,"/", "TEST6_quizcat_div_compare_linear_stats.csv", sep=""), row.names = FALSE)
