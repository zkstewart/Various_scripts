...
def generate_gc_file(fastaFile, windowSize, outputFile, amycneDirectory):
    '''
    Parameters:
    ...
    cmd = ZS_Utility.base_subprocess_cmd("python")
    cmd += [
...
def run_amycne_per_gene(gcFile, tidditFile, gff3Obj, outputFileName, amycneDirectory):
    '''
    Parameters:
...
def main():
    ...
    for each sample. NOTE: AMYCNE is only compatible with Python 2! This script will assume that
    the python discoverable in your system PATH is Python 2.7. If it's not, expect errors to occur.
    """
    ...
    if not os.path.exists(gcFile):
        generate_gc_file(args.fastaFile, args.windowSize, gcFile, args.amycneDirectory)
    else:
    ...
    if not os.path.exists(os.path.join(args.outputDirectory, "amycne_was_successful.flag")):
        ...
        for bamFile in bamFiles:
            ...
            if not os.path.exists(tidditFileName):
                generate_tiddit_file(args.fastaFile, bamFile, args.windowSize, tidditPrefix, args.tiddit)
            else:
            ...
            if not os.path.exists(amycneFileName):
                run_amycne_per_gene(gcFile, tidditFileName, gff3Obj, amycneFileName, args.amycneDirectory)
            else:
